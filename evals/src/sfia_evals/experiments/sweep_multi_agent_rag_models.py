...
from multi_agent_rag_matcher import SWEEP_MODELS
...
def score_role(matcher: MultiAgentRAGMatcher, role: dict) -> tuple:
...
def eval_model(model: str, roles: list[dict]) -> dict:
    matcher = MultiAgentRAGMatcher(model)
...
def main():
    ...
    for model in SWEEP_MODELS:
        ...
        new_rows.append({
            "implementation": "multi-agent-rag-crewai",
            "config": {"model": model},
            "metrics": metrics,
        })
